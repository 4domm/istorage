...
func NewVolumeHandler(store *Store, cfg VolumeConfig) http.Handler {
	...
	mux.HandleFunc("/internal/packs/", func(w http.ResponseWriter, r *http.Request) {
		...
		switch action {
		case "write-primary":
			handleBinaryWrite(w, r, func(req EntryWriteRequest, body []byte) error {
...
func handleBinaryWrite(w http.ResponseWriter, r *http.Request, fn func(req EntryWriteRequest, body []byte) error) {
	if r.Method != http.MethodPost {
		http.NotFound(w, r)
		return
	}
...
func postBinary(client *http.Client, baseURL, path string, req EntryWriteRequest, body []byte) error {
