...
async fn put_chunk(
    ...
    let db = db.clone();
    tokio::task::spawn_blocking(move || db.blocking_write().put(chunk_id.as_bytes(), data.as_ref()))
        .await
...
async fn get_chunk(
    ...
    let value = tokio::task::spawn_blocking(move || db.blocking_read().get(id.as_bytes()))
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))?
...
async fn delete_chunk(
    ...
    let db = db.clone();
    tokio::task::spawn_blocking(move || db.blocking_write().delete(chunk_id.as_bytes()))
        .await
        .map_err(|e| (StatusCode::INTERNAL_SERVER_ERROR, e.to_string()))??;
...
async fn main() -> anyhow::Result<()> {
    tracing_subscriber::registry()
        .with(tracing_subscriber::EnvFilter::try_from_default_env().unwrap_or_else(|_| "chunker=info,tower_http=debug".into()))
